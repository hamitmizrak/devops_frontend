...
// FUNCTION
function CategoryUpdate({t, i18n, props}) {
    ...
    useEffect(() => {
        ...
        // ID
        // 1.YOL localStorage.getItem("");
        setID(updateID.id);
        //FIND
        ApiCategory.categoryApiFindById(updateID.id)
            .then((response) => {
    ...
    // Update Change
    const updateCategoryOnChange = (event) => {
        //  name="category_name"
    ...
    // Update Submit
    const categoryUpdateSubmit = async (event) => {
        ...
        // try-catch
        try {
            const response = await ApiCategory.categoryApiUpdate(id,newCategory);
    ...
    } // end categoryUpdateSubmit

    // RETURN
    return (
        <React.Fragment>
            <h1 className="text-center display-3">{t("category_update")}</h1>
            <form>
                <div className="form-group">
                    ...
                    <input
                        ...
                        autoFocus={true}
                        required={true}
                        onChange={updateCategoryOnChange}
                        value={categoryName}
                        // 2.YOL
